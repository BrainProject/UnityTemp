...
public class MouseKeySimulator{
		...
		public static bool MouseCursorMove = true;

		private static Vector2 ConvertToScreen(Vector2 vect)
		{
			Vector2 result = new Vector2 (vect.x *Screen.width,(Screen.height - vect.y * Screen.height));
			return result;
		}

		//Convert window vect2 to screan coordinates and set position of cursor 
		public static void CursorPos(Vector2 vect)
		{
			if(MouseCursorMove)
			{
				Vector2 screen = ConvertToScreen(vect);
				SetCursorPos(Convert.ToInt32(screen.x),Convert.ToInt32(screen.y ));
				//Debug.Log(vect + " on " + new Vector2(Convert.ToInt32(vect.x *Screen.width),Screen.height - Convert.ToInt32(vect.y * Screen.height)));
			}
		}
