	...
	public class RotateAroundBrainBorder : MonoBehaviour {
		...
		void Start()
		{
			...
			case Side.Left:
				...
				this.GetComponent<GUITexture>().pixelInset = new Rect(-Screen.width/2, -Screen.height/2, Screen.width/4, Screen.height + 4);
				break;
			case Side.Right:
				rotatingAxis = Vector3.down;
				this.GetComponent<GUITexture>().pixelInset = new Rect(Screen.width/2 - Screen.width/4, - Screen.height/2, Screen.width/4, Screen.height + 4);
				break;
			case Side.Forward:
